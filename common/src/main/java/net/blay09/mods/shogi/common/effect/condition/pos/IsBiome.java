...
public record IsBiome(HolderSet<Biome> biome) implements ShogiEffect<Boolean> {
    ...
    public static final MapCodec<IsBiome> MAP_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            RegistryCodecs.homogeneousList(Registries.BIOME).fieldOf("biome").forGetter(it -> it.biome)
    ...
    public Either<Boolean, Throwable> apply(ShogiContext context) {
        ...
        return Either.left(biome.contains(level.getBiome(pos)));
    }
